// Simple fast pure TypeScript MD5 implementation for Gravatar hashing
...
// Helpers para extrair Informações do Remetente
...
const PASTEL_COLORS = [
  ...
  { bg: "bg-[#FFF0F5]", text: "text-[#D11A70]" }, // Rosa
  { bg: "bg-[#E8EAF6]", text: "text-[#3F51B5]" }, // Indigo
];

export function parseSenderDetails(fromStr: string): ParsedSenderInfo {
  if (!fromStr) {
    return {
      name: "Desconhecido",
      email: "",
  ...
  if (match) {
    ...
    email = cleanStr.replace(/[<>]/g, '').trim().toLowerCase();
    const userPart = email.split('@')[0];
    name = userPart.replace(/[._-]/g, ' ').replace(/\b\w/g, l => l.toUpperCase());
  } else {
    name = cleanStr;
    email = "";
  }
  ...
  // Cor determinística baseada no nome/email
  let hash = 0;
  const hashSource = email || name;
