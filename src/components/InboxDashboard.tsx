  ...
  const renderCanvasToggle = () => (
    <div className="w-full flex items-center justify-end mb-3 select-none">
      <button
        ...
        className={`px-3 py-1 rounded-full text-[11px] font-bold flex items-center gap-1.5 transition-all shadow-xs cursor-pointer border ${
          isDarkCanvas 
            ? 'bg-emerald-500/20 text-emerald-300 border-emerald-500/40 shadow-[0_0_10px_rgba(16,185,129,0.2)]' 
            : 'bg-zinc-100 dark:bg-white/10 text-zinc-700 dark:text-zinc-300 border-zinc-200 dark:border-white/10 hover:bg-zinc-200'
        }`}
        title="Alternar entre Tela Limpa Branca e Modo de Leitura Escura de Alto Contraste"
      >
        {isDarkCanvas ? (
          <>
            <Sun className="w-3.5 h-3.5 text-emerald-400" />
            <span>Leitura Escura Ativa</span>
          </>
        ) : (
          <>
            <Moon className="w-3.5 h-3.5 text-zinc-500 dark:text-zinc-400" />
            <span>Alternar Leitura Escura</span>
          </>
        )}
      </button>
    </div>
        ...
        <div 
          className={`email-rich-container rapimoney-reader-card rounded-[14px] p-4 md:p-6 shadow-2xl overflow-x-auto select-text cursor-text w-full max-w-full flex flex-col items-center transition-all duration-200 ${
            isDarkCanvas 
              ...
              : 'bg-white text-[#202124] border-zinc-200 dark:border-white/20'
          }`}
          style={isDarkCanvas ? { backgroundColor: "#0D1322", color: "#E2E8F0" } : undefined}
        >
          <div 
...
export function InboxDashboard({ user, initialEmails, currentFolder }: Props) {
  ...
  const handleSendAiDrawerMessage = async (customPrompt?: string) => {
    const textToSend = typeof customPrompt === 'string' ? customPrompt : aiDrawerPrompt;
    if (!textToSend.trim() || aiDrawerLoading) return;
  ...
  return (
    <div 
      className={`h-full min-h-[100dvh] w-full min-w-full overflow-hidden flex flex-col font-sans transition-colors duration-150 ${
        isLight ? 'bg-[#FFFFFF] text-[#202124]' : 'rapimoney-bg text-[#E8EAED]'
      }`}
      style={{ background: isLight ? "#FFFFFF" : "radial-gradient(circle at 15% 15%, #0d1527 0%, #060911 100%)", backgroundColor: isLight ? "#FFFFFF" : "#060911" }}
    >
      ...
      <header className={`h-14 border-b flex items-center justify-between px-3 md:px-5 z-20 shrink-0 transition-colors select-none ${
        isLight ? 'bg-[#FFFFFF] border-[#E5E7EB]' : 'rapimoney-header'
      }`}>
      ...
      {isMobileMenuOpen && (
        <div className="fixed inset-0 z-50 flex md:hidden bg-black/60 backdrop-blur-xs select-none">
          <div className="w-72 bg-white dark:bg-[#0D1527] h-full flex flex-col p-4 shadow-2xl animate-in slide-in-from-left duration-200 border-r border-white/10">
            <div className="flex items-center justify-between pb-4 border-b border-zinc-200 dark:border-white/10">
              <div className="flex items-center gap-2">
                <div className="w-7 h-7 rounded-lg bg-gradient-to-tr from-emerald-500 to-emerald-600 text-white flex items-center justify-center font-bold text-xs shadow-md">
                  <Mail className="w-4 h-4" />
                </div>
            ...
            <div className="py-4">
              <button 
                ...
                className="rapi-compose-btn w-full py-2.5 text-white font-bold text-xs rounded-[12px] flex items-center justify-center gap-2 shadow-md cursor-pointer"
              >
            ...
            <nav className="flex-1 space-y-1 overflow-y-auto">
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center justify-between px-3 py-2.5 text-xs transition-all ${
                      isActive 
                        ? 'rapimoney-sidebar-active' 
                        : 'text-zinc-700 dark:text-zinc-300 hover:bg-zinc-100 dark:hover:bg-white/5 rounded-xl'
                    }`}
                  >
                    <div className="flex items-center gap-3">
                      <Icon className={`w-4 h-4 ${isAgent ? 'text-indigo-400' : isActive ? 'text-[#10B981]' : ''}`} />
                      <span>{f.label}</span>
                    </div>
                    {isAgent ? (
                      <span className="text-[9px] font-extrabold px-1.5 py-0.5 rounded-full bg-gradient-to-r from-indigo-500/20 to-purple-500/20 text-indigo-300 border border-indigo-500/30 uppercase">
                        DO AI
                      </span>
                    ) : f.count > 0 ? (
                      <span className={isActive ? 'rapimoney-badge text-xs' : 'text-[10px] font-bold text-[#10B981]'}>{f.count}</span>
                    ) : null}
                  </button>
                );
              })}
              ...
              <div className="pt-2">
                <button
                  ...
                  className="w-full flex items-center justify-between px-3 py-2.5 text-xs rounded-xl bg-gradient-to-r from-indigo-500/15 to-purple-500/15 border border-indigo-500/30 text-indigo-300 hover:bg-indigo-500/25 transition-all shadow-xs"
                >
                  <div className="flex items-center gap-2.5">
                    <Globe2 className="w-4 h-4 text-indigo-400" />
                    <span className="font-semibold text-white">Criar Site com IA</span>
                  </div>
                  <span className="text-[9px] font-extrabold px-1.5 py-0.5 rounded-full bg-indigo-500/20 text-indigo-300 uppercase">
                    NOVO
                  </span>
                </button>
              </div>
            ...
            <div className="p-3 border-t border-zinc-200 dark:border-white/10 text-xs shrink-0">
              <div className="flex items-center justify-between mb-1 text-[11px] font-semibold text-zinc-700 dark:text-zinc-300">
                <span>Armazenamento</span>
                <span className="text-[10px] text-emerald-400 font-mono font-bold">{storageInfo.percentage}%</span>
              </div>
              <div className="w-full rapimoney-storage-track overflow-hidden mb-1">
                <div className="h-full rapimoney-storage-fill transition-all duration-300" style={{ width: storageInfo.barWidthPercentage }}></div>
              </div>
              <span className="text-[10px] text-zinc-400 block">{storageInfo.usedDisplay} de {storageInfo.limitDisplay} ({storageInfo.percentage}%)</span>
            </div>
      ...
      {activeTab === 'mail' && (
        <div className="flex-1 flex overflow-hidden min-h-0 relative">
          ...
          <aside className={`hidden md:flex w-[210px] border-r flex-col shrink-0 h-full overflow-hidden transition-colors select-none ${
            ...
            <div className="p-3">
              <button 
                ...
                className="rapi-compose-btn w-full flex items-center justify-center gap-2 text-white py-2.5 px-4 rounded-[12px] font-bold text-xs shadow-md transition-all duration-250 cursor-pointer"
              >
                <span>Escrever</span>
              </button>
            </div>
            ...
            <nav className="flex-1 px-2 py-1 space-y-0.5 overflow-y-auto">
                    ...
                    className={`w-full flex items-center justify-between px-3 py-2 text-xs transition-all ${
                      isActive 
                        ...
                        : isLight 
                          ...
                          : 'text-zinc-400 hover:text-white rapimoney-email-card-hover rounded-r-xl'
                    }`}
                  >
                    <div className="flex items-center gap-2.5">
                      <Icon className={`w-4 h-4 ${isAgent ? 'text-indigo-400' : isActive ? 'text-[#10B981]' : isLight ? 'text-[#5F6368]' : 'text-zinc-400'}`} />
                    ...
                    {isAgent ? (
                      ...
                      <span className={isActive ? 'rapimoney-badge text-xs' : 'text-[11px] font-bold text-zinc-400'}>
                        {folder.count}
                      </span>
                    ) : null}
                  </button>
              ...
              <div className="pt-2 px-1">
                <button
                  ...
                  className="w-full flex items-center justify-between px-2.5 py-2 text-xs rounded-xl bg-gradient-to-r from-indigo-500/10 to-purple-500/10 hover:from-indigo-500/20 hover:to-purple-500/20 border border-indigo-500/20 text-zinc-300 hover:text-white transition-all shadow-xs group cursor-pointer"
                >
                  <div className="flex items-center gap-2">
                    <Globe2 className="w-3.5 h-3.5 text-indigo-400 group-hover:scale-110 transition-transform" />
                    <span className="font-medium">Criar Site com IA</span>
                  </div>
                  <span className="text-[9px] font-extrabold px-1.5 py-0.2 rounded-full bg-indigo-500/20 text-indigo-300 uppercase">
                    AI
                  </span>
                </button>
              </div>
            ...
            <div className={`p-3 border-t text-xs shrink-0 ${isLight ? 'border-[#E5E7EB] bg-[#FFFFFF]' : 'border-white/[0.07] bg-black/20'}`}>
          ...
          <section className={`${
            ...
            <div className="flex-1 overflow-y-auto divide-y divide-[#E5E7EB] dark:divide-white/[0.04]">
              {filteredEmails.length === 0 ? (
                <div className="p-8 text-center text-zinc-400 text-xs">
                  <Inbox className="w-8 h-8 mx-auto mb-2 opacity-40 text-emerald-400" />
                            ...
                            <div className="flex items-center gap-1.5 shrink-0 ml-2">
                              ...
                              {hasAttachment && (
                                <span title={`${emailAttachments.length} ${emailAttachments.length === 1 ? 'Anexo' : 'Anexos'} anexado(s)`}>
                                  <Paperclip className="w-3.5 h-3.5 text-[#10B981] dark:text-emerald-400 shrink-0" />
                                </span>
                              )}
                              ...
                              {isSent && (
                                email.isOpened ? (
                                  <span title={`✅ Lido pelo destinatário em ${email.openedAt ? new Date(email.openedAt).toLocaleTimeString('pt-PT', { hour: '2-digit', minute: '2-digit' }) : ''} (${email.openCount || 1}x no ${email.userAgent || 'dispositivo'})`}>
                                    <CheckCheck className="w-3.5 h-3.5 text-[#10B981] dark:text-emerald-400" />
                                  </span>
                                ) : (
                              ...
                              <span 
                                ...
                                className={`text-[10px] font-mono ${
                                  isUnread ? 'font-bold text-[#10B981]' : 'text-zinc-400'
                                }`}
                              >
                                {dateDisplay}
                              </span>
            ...
            {selectedEmail ? (
              <div className="flex-1 flex flex-col h-full overflow-hidden min-h-0">
                ...
                <div className="flex-1 overflow-y-auto p-4 pb-36 md:p-8 md:pb-24 w-full select-text overscroll-contain">
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-3 p-3 md:p-3.5 rapimoney-ai-banner shadow-xs select-none animate-in fade-in duration-150">
                    <div className="flex items-center gap-2.5 min-w-0">
                      <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-indigo-600 to-emerald-600 text-white flex items-center justify-center shadow-xs shrink-0">
                      ...
                      <div className="min-w-0">
                        <div className="flex items-center gap-1.5 flex-wrap">
                          ...
                          <span className="rapimoney-ai-badge text-[9px] uppercase">
                            DO & RapiAI Engine
                          </span>
                        </div>
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                      <button
                        ...
                        className="px-3 py-1.5 rapimoney-ai-btn text-xs font-bold flex items-center gap-1.5 transition-all shadow-xs active:scale-95 cursor-pointer disabled:opacity-50"
                      >
                        <Sparkles className={`w-3.5 h-3.5 text-white ${isGeneratingSmartReply ? 'animate-spin' : ''}`} />
                      ...
                      <button
                        ...
                        className="px-3 py-1.5 bg-white/10 hover:bg-white/15 text-white border border-white/10 rounded-xl text-xs font-bold flex items-center gap-1.5 transition-all shadow-xs active:scale-95 cursor-pointer disabled:opacity-50"
                      >
                        <ListTodo className={`w-3.5 h-3.5 text-emerald-400 ${isSummarizing ? 'animate-spin' : ''}`} />
                      ...
                      <button
                        ...
                        className="px-3 py-1.5 bg-white/10 hover:bg-white/15 text-white border border-white/10 rounded-xl text-xs font-bold flex items-center gap-1.5 transition-all shadow-xs active:scale-95 cursor-pointer"
                      >
          ...
          <div className="w-full max-w-md bg-white dark:bg-[#0F1118] h-full flex flex-col shadow-2xl border-l border-[#E5E7EB] dark:border-white/10 animate-in slide-in-from-right duration-200">
            ...
            <div className="px-5 py-4 border-b border-[#E5E7EB] dark:border-white/10 bg-zinc-50 dark:bg-[#0B0D13] flex items-center justify-between">
              ...
              <button 
                ...
                className="p-1.5 text-zinc-400 hover:text-zinc-700 dark:hover:text-white rounded-lg hover:bg-zinc-200/50 dark:hover:bg-white/10 transition-colors cursor-pointer"
              >
                <X className="w-5 h-5" />
              </button>
            </div>

            {/* 📌 Banner de Contexto do E-mail Ativo */}
            {selectedEmail && (
              <div className="px-4 py-2.5 bg-gradient-to-r from-indigo-50/90 to-purple-50/80 dark:from-indigo-950/40 dark:to-purple-950/30 border-b border-indigo-100 dark:border-indigo-500/20 flex items-center justify-between gap-2 text-xs">
                <div className="flex items-center gap-2 min-w-0">
                  <span className="w-2 h-2 rounded-full bg-emerald-500 shrink-0" />
                  <span className="font-bold text-indigo-900 dark:text-indigo-200 truncate">
                    E-mail em análise: {parseSenderDetails(selectedEmail.from).name}
                  </span>
                  <span className="text-[11px] text-zinc-500 dark:text-zinc-400 truncate max-w-[140px]">
                    ({selectedEmail.subject})
                  </span>
                </div>
                <span className="text-[9px] uppercase font-bold px-1.5 py-0.5 rounded bg-indigo-500/20 text-indigo-700 dark:text-indigo-300 border border-indigo-500/30 shrink-0">
                  Contexto Ativo
                </span>
              </div>
            )}
            ...
            <div className="p-3 border-b border-[#E5E7EB] dark:border-white/5 bg-zinc-50/50 dark:bg-black/20 flex gap-2 overflow-x-auto text-[11px] no-scrollbar">
              {selectedEmail ? (
                <>
                  ...
                  <button
                    onClick={() => {
                      handleSendAiDrawerMessage("Resume os pontos críticos e exigências deste e-mail de forma concisa.");
                    }}
                    className="px-2.5 py-1 rounded-full bg-white dark:bg-white/5 border border-zinc-200 dark:border-white/10 text-zinc-700 dark:text-zinc-300 hover:border-indigo-500 shrink-0 transition-colors cursor-pointer"
                  >
                    📋 Resumo dos Pontos
                  </button>
                  <button
                    onClick={() => {
                      handleSendAiDrawerMessage("Redige uma resposta a solicitar prazo até amanhã para finalizar a análise jurídica e enviar a documentação.");
                    }}
                    className="px-2.5 py-1 rounded-full bg-white dark:bg-white/5 border border-zinc-200 dark:border-white/10 text-zinc-700 dark:text-zinc-300 hover:border-indigo-500 shrink-0 transition-colors cursor-pointer"
                  >
                    ⏳ Pedir Prazo (Amanhã)
              ...
              <button
                onClick={() => {
                  handleSendAiDrawerMessage("Quero criar um website para o meu negócio com o assistente de IA. Como funciona?");
                  setIsSiteBuilderOpen(true);
                }}
                className="px-2.5 py-1 rounded-full bg-gradient-to-r from-indigo-500/20 to-purple-500/20 border border-indigo-500/40 text-indigo-300 hover:bg-indigo-500/30 shrink-0 transition-colors font-semibold cursor-pointer flex items-center gap-1.5 shadow-xs"
              >
              ...
              <button
                onClick={() => {
                  handleSendAiDrawerMessage("Escreve uma proposta comercial executiva B2B sob medida.");
                }}
                className="px-2.5 py-1 rounded-full bg-white dark:bg-white/5 border border-zinc-200 dark:border-white/10 text-zinc-700 dark:text-zinc-300 hover:border-indigo-500 shrink-0 transition-colors cursor-pointer"
              >
                💼 Proposta Comercial
              ...
              <button
                onClick={() => {
                  handleSendAiDrawerMessage("Cria um lembrete educado para pagamento de fatura pendente.");
                }}
                className="px-2.5 py-1 rounded-full bg-white dark:bg-white/5 border border-zinc-200 dark:border-white/10 text-zinc-700 dark:text-zinc-300 hover:border-indigo-500 shrink-0 transition-colors cursor-pointer"
              >
                💳 Cobrança Educada
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3.5 select-text">
              {aiDrawerMessages.map((msg, index) => {
                ...
                const hasDraftText = !isUser && (
                  msg.text.includes("Estimado") || 
                  msg.text.includes("Olá") || 
                  msg.text.includes("Cumprimentos") || 
                  msg.text.includes("Atenciosamente")
                );

                return (
                  <div key={index} className={`flex flex-col ${isUser ? 'items-end' : 'items-start'}`}>
                    <div className={`max-w-[92%] p-3.5 rounded-2xl text-xs leading-relaxed ${
                      isUser 
                      ...
                      {/* Ação Direta para Abrir Criador de Sites */}
                      {!isUser && (msg.text.includes("Criador de Websites") || msg.text.includes("RapiSiteBuilder") || msg.text.includes("desenhar e publicar um website")) && (
                        <div className="mt-3 pt-2.5 border-t border-zinc-200/70 dark:border-white/10 flex items-center gap-2 select-none">
                          <button
                            type="button"
                            onClick={() => {
                            ...
                            className="px-3 py-1.5 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 text-white rounded-xl font-bold text-xs flex items-center gap-1.5 shadow-md shadow-indigo-600/20 transition-all cursor-pointer active:scale-95"
                          >
                            <Globe2 className="w-3.5 h-3.5 text-white" />
                            <span>🚀 Abrir Criador de Sites com IA</span>
                          </button>
                        </div>
                      )}

                      {/* Ações de 1-Clique na Resposta Gerada */}
                      {hasDraftText && (
                        <div className="mt-3 pt-2.5 border-t border-zinc-200/70 dark:border-white/10 flex items-center gap-2 flex-wrap select-none">
                          <button
                            type="button"
                            onClick={() => {
                              const sender = parseSenderDetails(selectedEmail?.from || "");
                              ...
                              setIsAiDrawerOpen(false);
                              setToastMessage("✉️ Resposta inserida no editor com sucesso!");
                              setTimeout(() => setToastMessage(null), 3000);
                            }}
                            className="px-2.5 py-1 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg font-bold text-[11px] flex items-center gap-1 shadow-xs transition-all cursor-pointer active:scale-95"
                          >
                            <Reply className="w-3 h-3" />
                            <span>Inserir no Editor de Resposta</span>
                          </button>
                          ...
                          <button
                            ...
                            onClick={() => {
                              ...
                              navigator.clipboard.writeText(draftBody.trim());
                              setToastMessage("📋 Resposta copiada para a área de transferência!");
                              setTimeout(() => setToastMessage(null), 3000);
                            }}
                            className="px-2 py-1 bg-white dark:bg-white/10 border border-zinc-200 dark:border-white/10 hover:bg-zinc-50 text-zinc-700 dark:text-zinc-200 rounded-lg font-semibold text-[11px] flex items-center gap-1 transition-all cursor-pointer"
                          >
              ...
              {aiDrawerLoading && (
                <div className="flex items-center gap-2 p-3 bg-indigo-50 dark:bg-indigo-950/30 border border-indigo-200 dark:border-indigo-500/20 rounded-2xl text-xs text-indigo-600 dark:text-indigo-300 animate-pulse">
                  ...
                  <span>O Agente RapiAI está a analisar o e-mail e a formular a resposta...</span>
                </div>
              )}
            </div>
            ...
            <div className="p-3.5 border-t border-[#E5E7EB] dark:border-white/10 bg-white dark:bg-[#0B0D13]">
              <div className="flex items-center gap-2 bg-zinc-100 dark:bg-white/5 border border-zinc-200 dark:border-white/10 rounded-2xl px-3 py-1.5 focus-within:border-indigo-500 transition-all">
                <input
                  ...
                  placeholder={selectedEmail ? `Pergunte sobre o e-mail de ${parseSenderDetails(selectedEmail.from).name.split(' ')[0]}...` : "Pergunte ao agente ou peça um rascunho..."}
                ...
                <button
                  ...
                  className="p-2 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-40 text-white rounded-xl transition-all shadow-xs cursor-pointer"
                >
